...
def calculate_subtotal(order):
    ...
    sub_sum = 0
    for item in order:
        sub_sum += float(item['price'])
    return round(sub_sum, 2)
...
def summarize_order(order):
    ...
    [IMPLEMENT ME]
        1. Calculate the total (subtotal + tax) and store it in a variable named total
    ...
    total = round(sub_total + calculate_tax(sub_total), 2)
